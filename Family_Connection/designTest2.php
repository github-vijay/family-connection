...
<nav class="navbar navbar-inverse navbar-fixed-top" style="margin:0px;">
  <div class="container-fluid">
    ...
    <div class="navbar-header">
      ...
      <a class="navbar-brand navbar-brand-mod" href="designTest2.php">FAMILY CONNECTION</a>
    </div>
...
<div id="user-info">
	<div id="profile-photo">
		<!-- <img src="<?php echo $_SESSION['u_info']['Profile_Pic']?>" alt="<?php echo $_SESSION['u_info']['First_Name']; ?>"> -->
	...
	<div id="cover-photo">
		<!-- <img src="<?php echo $_SESSION['u_info']['Cover_Pic']?>" alt="<?php echo $_SESSION['u_info']['First_Name']; ?>"> -->
	</div>
	<span id="user-name">
	<?php 
...
<div id="feeds">
	...
	if(mysqli_num_rows($result) > 0) {
	    ...
	    while($row = mysqli_fetch_array($result)) { 
	    	$sql = "SELECT `First_Name`,`Middle_Name`,`Last_Name`,`Profile_Pic` from `user` where `ID` = '".$row['User_ID']."'";
			$query_for_name = mysqli_query($conn,$sql);
			$name = mysqli_fetch_array($query_for_name);
		?>
			<div class="feed-box">
				<div class="feed-box-header">
					<div class="feed-box-header-image">
						<img src="<?php echo $name['Profile_Pic'];?>" alt="<?php echo $name['First_Name'];?>" width="80" height="78">
					</div>
					<div class="feed-box-user-name">
						...
						<span>Posted on <?php echo date("F j, Y, g:i a", $row["Upload_Time"]);?></span>
					</div>
				</div>
	    ...
	    <?php
	    		...
	    		if(strlen($row['Image']))
	    			echo '<div class="feed-box-image-div"><img class="img-responsive feed-box-image" src="'.$row['Image'].'"/></div>';
    	 ...
    	 echo "<h3>Start making friends or post status of your own to see something here.</h3>";
    }    	
    $conn->close();
?>
