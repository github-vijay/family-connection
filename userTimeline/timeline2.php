...
<script type="text/javascript">
	function displayUpload(){
		document.getElementById("upload_prof_pic").style.display = "inline-block";
	}
</script>
</head>
...
<div id="user-info">
	<div id="profile-photo">
		<div class="fileinputs">
			<form id="prof_form" method="POST" action="profileImageAction.php" enctype="multipart/form-data">
				<input type="file" name="prof_file" class="prof_file" onclick="displayUpload()">
				<div class="fakefile">
					<span style="position: relative;top:-24px;">Select Profile Pic</span>
				</div>
				<input type="submit" id="upload_prof_pic" name="change_profile_img" value="Upload" style="width: 100%;position: relative;top: 6px;display: none;">
			</form>
		</div>
		<!-- <form method="POST" action="profileImageAction.php" enctype="multipart/form-data">
			<span>Change Profile Pic<input type="file" name="profile_img" accept="image/*" required ></span>
...
<!-- This script locds the profile picture -->
<script type="text/javascript">
        var request = $.ajax({
        ...
        request.done(function( response ) {
           ...
           $('.feed-box-header-image').css("background-image", "url("+ response +")"); 
        });
...
<!-- This script loads the cover picture -->
<script type="text/javascript">
        var request = $.ajax({
          url: "profilePictureLoad.php",
          method: "POST",
...
<!-- THis script update the Online Status of user in database -->
<script type="text/javascript">
window.setInterval(updateUserOnlineStatus,5000);
function updateUserOnlineStatus(){
	var updateOnlineStatusRequest = $.ajax({
		url : "updateUserOnlineStatus.php",
		method : "GET",
		data : {"onlineStatus" : "2"} //2 means user is on timeline
	});

	updateOnlineStatusRequest.done(function( response ){
		// console.log(response);
	});

	updateOnlineStatusRequest.fail(function( jqXHR, textStatus ){
		// console.log("Could not update online status.");
	});
}
</script>
<!-- This script update the number of unseen messages of user -->
<script type="text/javascript">
window.setInterval(getUnseenMessagesCount,5000);

function getUnseenMessagesCount(){
	var request = $.ajax({
		url : "getUnseenMessagesCount.php",
		method : "GET",
		data : {"current_time" : $.now()}
	});

	request.done(function( response ){
		document.getElementById("unseen_messages").innerHTML = response;
		if(response > 0)
			document.getElementsByClassName("glyphicon glyphicon-bell")[0].style.backgroundColor = "#00f910";
		else
			document.getElementsByClassName("glyphicon glyphicon-bell")[0].style.backgroundColor = "transparent";
	});
